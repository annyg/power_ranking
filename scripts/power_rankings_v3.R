...
matches <- matches_serie %>%
  # bind_rows(matches_obos) %>%  # Combine both datasets
  ...
  select(Dato,Runde, week, Hjemmelag, Bortelag, Hjemme, Borte) %>%  # Select relevant columns
  rename(
...
team_stats_by_round <- matches %>%
  ...
  pivot_longer(
    cols = c(team_a, team_b, score_a, score_b, score_against_a, score_against_b, gd_a, gd_b, points_a, points_b),
  ...
  group_by(team) %>%  # Group by team_type and week
  complete(
...
# team_strength <- team_stats_by_round %>%
#   arrange(match) %>%
#   group_by(team, week) %>% #TODO something is wrong here
#   # mutate(
