...
# --- Put this right after you've built `matches_with_strength` --- UPDATE

# Work out season order from earliest kickoff in each 'Turnering'
season_order <- matches_with_strength %>%
  group_by(Turnering) %>%
  summarise(season_start = min(Dato, na.rm = TRUE), .groups = "drop") %>%
  arrange(season_start) %>%
  pull(Turnering)

# Ensure we process earlier seasons before later seasons (øst/vest → høst)
matches_with_strength <- matches_with_strength %>%
  mutate(Turnering = factor(Turnering, levels = season_order)) %>%
  arrange(Turnering, Dato, match)
...
team_stats_by_round_w_strength <- team_stats_by_round %>%
  ...
  mutate(
    ...
    # team_elo = case_when(week == lubridate::isoweek(date_season_start) ~ 1500, TRUE ~ team_elo),  # Initialize Elo rating for week 13) UPDATE
